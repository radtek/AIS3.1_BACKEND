...
//	}
	...
	public static void checkMonitorData(ChannelHandlerContext ctx,Map<String, Object> obj) {
		...
		logger.info("checkMonitorData------采集服务IP:" + address.getHostString()+" port:"+address.getPort());
		...
		**/
		...
		if (newObserves != null && newObserves.size() > 0) {
        	BasOperroomService basOperroomService = SpringContextHolder.getBean(BasOperroomService.class);
        	String beid = basOperroomService.getBeidByIpPort(ip, port);
        	String operroomId = basOperroomService.getRoomIdByIp(ip, beid);
        	logger.info("checkMonitorData------当前手术室:operroomId" + operroomId);
        	if(StringUtils.isNotBlank(operroomId) && !"0".equals(operroomId)){
    			...
    			json.put("time", timeFormat);
    			logger.info("checkMonitorData---operroomId---"+operroomId+"----time------" + time);
    			BasObserveService observeService = SpringContextHolder.getBean(BasObserveService.class);
    			String key = beid+operroomId;
    			//OperationMgr.regOptIdMap根据局点ID+床位ID获取regOptId
    			if(StringUtils.isNotBlank(OperationMgr.regOptIdMap.get(key))){
	    			regOptId = OperationMgr.regOptIdMap.get(key);
	    			
	    			String roomId = observeService.getCurRoomId(regOptId);
	    			
	    			Map<String, Observe> monitors = DataMonitor.getObservesByRoom(roomId, regOptId);
	    			java.util.Iterator<Observe> ite = newObserves.iterator();
	    			
	    			//获取bas_monitor_config表中对应roomId为当前手术室的数据
	    			Map<String, BasMonitorConfig> monitorConfigItems = DataMonitor.getMonitorConfigByRoomId(beid, roomId);
	    			
	    			while (ite.hasNext()) {
	    				logger.info("-------checkMonitorData---newObserves-------while------regOptId:"+regOptId);
	    				...
	    				data.setRegOptId(regOptId);// 从OperationMgr中获取regOptId
	    				...
	    				// 增加units
	    				//BasObserveService observeService = SpringContextHolder.getBean(BasObserveService.class);
	    				BasMonitorConfig monitorConfig = monitorConfigItems.get(newObserve.getObserveId());//observeService.searchMonitorConfig(newObserve.getObserveId(), null,regOptId);
		...
		if(null != listJson && listJson.size()>0){
			...
			// 插入采集数据
			logger.info("checkMonitorData-----获取到采集数据"+"---对应采集服务IP:"+ip+",port:"+port);
			...
			if (null != rec.getObsDatas() && rec.getObsDatas().size() > 0) {
				...
				int count = observeDataService.searchMonitorDisplayByTime(regOptId, json.getString("time"));
		...
		logger.info("checkMonitorData------采集服务IP:" + ip+" port:"+port);
		logger.info("checkMonitorData----执行完成---regOptId:"+regOptId);
	}
    ...
    public static JSONObject updateObserDataJson(BasObserveData data) {
        ...
        // 获取Observe,增加units
        BasMonitorConfig monitorConfig = observeService.searchMonitorConfig(data.getObserveId(),null,data.getRegOptId());
        data.setUnits(monitorConfig.getUnits());
