...
//        }
    ...
    public static List<Device> getDevicesByRoom(String roomId) {
        ...
        // 根据业务需求，不传递roomId给接口
        List<Device> devices = deviceService.searchDeviceListByRoomId(roomId,null);
        return devices;
    }
     ...
     * @return
     */
    public static Map<String, BasMonitorConfig> getMonitorConfigByRoomId(String beid ,String roomId) {
        logger.info("getMonitorConfigByRoomId------获取监测项数据，roomId:" + roomId);
        List<BasMonitorConfig> monitorConfigList= SpringContextHolder.getBean(BasMonitorConfigDao.class).queryAllMonitorConfig(beid, roomId);
        Map<String, BasMonitorConfig> monitorItems = new HashMap<String, BasMonitorConfig>();
        if (!SysUtil.isEmptyList(monitorConfigList)) {
            Iterator<BasMonitorConfig> itemMonitorConfig = monitorConfigList.iterator();
            while (itemMonitorConfig.hasNext()) {
            	BasMonitorConfig monitor = itemMonitorConfig.next();
                String eventId = monitor.getEventId();
                monitorItems.put(eventId,monitor);
            }
        }
        logger.info("getMonitorConfigByRoomId------获取监测项数据，roomId:" + roomId
                + " monitorItems:" + JSONArray.fromObject(monitorItems).toString());
    ...
    public static String getMonitorFreq(String type) {
        ...
        BasMonitorConfigFreq monFreq = freqSer.searchMonitorFreqByType(type,null);
        String freq = "";
        if (monFreq != null) {
