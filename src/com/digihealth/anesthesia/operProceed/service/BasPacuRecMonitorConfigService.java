 ...
 */
@Service
public class BasPacuRecMonitorConfigService extends BaseService {
	...
	public List<BasAnaesMonitorConfigFormBean> getAnaesRecordShowListByRegOptId(BaseInfoQuery baseQuery) {
		...
		List<BasAnaesMonitorConfigFormBean> anaesMonitorList = basPacuRecMonitorConfigDao.getPacuRecRecordShowListByRegOptId(baseQuery);
...
//
	 ...
	 */
	@Transactional
	public void savePacuRecMonitorConfig(List<BasPacuRecMonitorConfig> pacuRecMonitorConfigList) {

		if (pacuRecMonitorConfigList.size() > 0) {
			BasPacuRecMonitorConfig queryObj = pacuRecMonitorConfigList.get(0);
			basPacuRecMonitorConfigDao.delete(queryObj);

			for (BasPacuRecMonitorConfig basPacuRecMonitorConfig : pacuRecMonitorConfigList) { 
				// if(logger.isDebugEnabled()){
				logger.debug("savePacuRecMonitorConfig data:" + basPacuRecMonitorConfig.toString());
				// }
				String beid = basPacuRecMonitorConfig.getBeid();
				if(StringUtils.isBlank(beid)){//beid判空
					basPacuRecMonitorConfig.setBeid(getBeid());
					beid = getBeid();
				}
				if (StringUtils.isEmpty(basPacuRecMonitorConfig.getId()))
				{
					basPacuRecMonitorConfig.setId(GenerateSequenceUtil.generateSequenceNo()); 
				}
				basPacuRecMonitorConfigDao.insert(basPacuRecMonitorConfig);
...
//
	public List<BasAnaesMonitorConfigFormBean> finaAnaesList(BaseInfoQuery baseQuery) {
		...
		List<BasAnaesMonitorConfigFormBean> anaesMonitorList = basPacuRecMonitorConfigDao.getPacuRecMonitorConfigListByRegOptId(baseQuery);
		return anaesMonitorList;
	}
//
