...
public class MyObserveDataController extends BaseController {
	...
	public static String ECG_EVENT_ID = "91003";//心电图
     ...
     * @return
     */
    @RequestMapping("/updobsdatQNZZY")
    @ResponseBody
    @ApiOperation(value = "修改点", httpMethod = "POST", notes = "修改点")
    public String updobsdatQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDisplayChangeFormBean changeBean) {
        logger.info("----------------start updobsdat------------------------");
        ResponseValue res = new ResponseValue();
        basMonitorDisplayService.changeMonitDisplayQNZZY(changeBean);
        res.setResultCode("1");
        res.setResultMessage("操作成功");
        logger.info("------------------end updobsdat------------------------");
        return res.getJsonStr();
     ...
     * @return
     */
    @RequestMapping("/getrtDataQNZZY")
    @ResponseBody
    @ApiOperation(value = "获取实时数据和获取设备状态", httpMethod = "POST", notes = "获取实时数据和获取设备状态")
    public String getrtDataQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody BaseInfoQuery baseQuery) {
        logger.info("----------------start getrtData------------------------");
        ResponseValue res = new ResponseValue();
        if (null != baseQuery) {
            // List<RealTimeDataFormBean> monitorList =
            ...
            Map<String, RealTimeDataFormBean> resultMap = basMonitorDisplayService.searchObserveMapByPositionQNZZY(baseQuery);
            res.put("monitorList", resultMap);
            // 获取设备列表
            List<Device> devices = basDeviceService.searchDeviceListByRoomId(basDeviceService.getCurRoomId(null), baseQuery.getBeid());
            res.put("devices", devices);
	...
	public Map getobsData(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
		...
		if (null != anaesLists && anaesLists.size() > 0) {
			for (BasAnaesMonitorConfigFormBean bean : anaesLists) {
				...
				int widthAndHeight = bean.getWidthAndHeight();
				if(widthAndHeight==0){
					widthAndHeight = 12;
				}
				...
				if (observeId.equals(NP_TEMP_POSITION_RT) || observeId.equals(RE_TEMP_POSITION_RT) || observeId.equals(NP_TEMP_POSITION_LINE) || observeId.equals(RE_TEMP_POSITION_LINE) || observeId.equals(TEMP_POSITION_RT) || observeId.equals(TEMP_POSITION_LINE)) { // 如果是温度，则y轴为2
					seriesMap.put(observeId, new SeriesData(observeId, observeName, "line", new ArrayList<SeriesDataObj>(), icon, svg, 1, widthAndHeight, color, units, max, min));
				} else if (observeId.equals(CVP_MEAN_POSITION_RT) || observeId.equals(CVP_MEAN_POSITION_LINE)) { // cvp,则y轴为1
					seriesMap.put(observeId, new SeriesData(observeId, observeName, "line", new ArrayList<SeriesDataObj>(), icon, svg, 2, widthAndHeight, color, units, max, min));
				} else {// abp_hp等
					seriesMap.put(observeId, new SeriesData(observeId, observeName, "line", new ArrayList<SeriesDataObj>(), icon, svg, 0, widthAndHeight, color, units, max, min));
				}
	...
	public Map getobsDataNew(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
		...
		BasMonitorConfigFreq monitorFreq = basMonitorConfigFreqService.searchMonitorFreqByType(currentModel,formBean.getRegOptId());
		String f = "";
		Integer freq = 0;
		if (monitorFreq != null) {
			f = monitorFreq.getValue();
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/getobsDataNewQNZZY")
    @ResponseBody
    @ApiOperation(value = "获取新点", httpMethod = "POST", notes = "获取新点")
    public Map getobsDataNewQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
        logger.info("----------------start getobsDataNew----------患者ID:"+formBean.getRegOptId());
        Map map = new HashMap();
        if (StringUtils.isBlank(formBean.getBeid())) {
            formBean.setBeid(getBeid());
        }
        ...
        List<BasMonitorDisplay> monitorList = basMonitorDisplayService.getobsDataNew4QNZZY(formBean, observeIds); // 增加特殊处理逻辑
        ...
        map.put("resultCode", "1");
        map.put("resultMessage", "操作成功！");
        logger.info("------------------end getobsDataNew---allData===" + JsonType.jsonType(map));
        logger.info("------------------end getobsDataNew--------患者ID:"+formBean.getRegOptId());
        return map;
    }
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/getIntervalObsDataQNZZY")
    @ResponseBody
    @ApiOperation(value = "页面再次打开，间隔点的处理", httpMethod = "POST", notes = "页面再次打开，间隔点的处理")
    public Map getIntervalObsDataQNZZY(@ApiParam(name = "formbean", value = "参数")@RequestBody IntervalDataFormBean formbean) {
        logger.info("----------------start getIntervalObsData------------------------");
        ResponseValue res = new ResponseValue();
        Map map = new HashMap();
        if (formbean != null) {
        	
        	logger.info("----------------start getIntervalObsData--------患者ID:"+formbean.getRegOptId());

        	logger.info("----------------getIntervalObsData times--------患者ID:"+formbean.getRegOptId() + "times: " + formbean.getTimes());
     ...
     * @return
     */
    @RequestMapping("/updDeviceConfigQNZZY")
    @ResponseBody
    @ApiOperation(value = "修改设备配置", httpMethod = "POST", notes = "修改设备配置")
    public String updDeviceConfigQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody DeviceConfigFormBean deviceConfigFormBean) {
        logger.info("----------------start updDeviceConfig------------------------");
        ResponseValue res = new ResponseValue();
        if (deviceConfigFormBean != null) {
            ...
            basMonitorDisplayService.updDeviceConfigQNZZY(deviceConfigFormBean);
            // 根据房间和术中状态查询手术
           /* BasRegOpt regOpt = basRegOptService.queryRegOptByState("04");
            if (null != regOpt) {
                // 发送修改设备配置的命令给数据处理模块
    ...
    public String startOper(@RequestBody SearchFormBean searchBean){
        ...
        try {
            ...
            if(StringUtils.isNotBlank(searchBean.getAccessSource())){
                ...
                {
                    BasOperroom operroom = basOperroomService.selectEntityByPrimaryKey(dispatch.getOperRoomId().toString());
            ...
            BasMonitorConfig O2 = basMonitorConfigService.selectByEventId(MyObserveDataController.O2_EVENT_ID,regOptId);
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/startOperQNZZY")
    @ResponseBody
    public String startOperQNZZY(@RequestBody SearchFormBean searchBean){
        logger.info("----------------start startOper------------------------");
        ...
        try {
            ...
            if(StringUtils.isNotBlank(searchBean.getAccessSource())){
                ...
                if(flagInsert){
                    ...
                    BasCollectConfig collectConfig = basCollectConfigService.selectByRoomId(operRoomId);
                    collectConfig.setPatientId(regOptId);
                    basCollectConfigService.update(collectConfig);
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/startOperLLZY")
    @ResponseBody
    public String startOperLLZY(@RequestBody SearchFormBean searchBean){
        logger.info("----------------start startOperLLZY------------------------");
        Map result = new HashMap();
        try {
        	String regOptId = searchBean.getRegOptId();
            //麻醉记录单
            ...
            if(StringUtils.isNotBlank(searchBean.getAccessSource())){
                ...
                if(flagInsert){
                    ...
                    List<EvtRealAnaesMethod> realList = evtRealAnaesMethodService.searchRealAnaesMethodList(searchBean);
                    if(realList.size()<1){
                    	//初始化麻醉方法时,从麻醉前评估和麻醉计划文书取麻醉方法
                    	PreVisitFormBean preVisit = docPreVisitService.searchPreVisitByRegOptId(regOptId);
                    	if (preVisit.getPreVisit() != null) {
                    		if(StringUtils.isNotBlank(preVisit.getPreVisit().getAnaesMethodCode())){
                    			String[] methodArray = preVisit.getPreVisit().getAnaesMethodCode().split(",");
                    			String[] methodNameArray = preVisit.getPreVisit().getAnaesMethod().split(",");
                    			for (int i = 0; i < methodArray.length; i++) {
                    				String anaMedId = methodArray[i];
                    				String realAnaMedName = methodNameArray[i];
                    				EvtRealAnaesMethod realAnaesMethod = new EvtRealAnaesMethod();
     ...
     * @return
     */
    @RequestMapping("/endOperationQNZZY")
    @ResponseBody
    @ApiOperation(value = "正常结束手术", httpMethod = "POST", notes = "正常结束手术")
    public String endOperationQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start endOperation------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        logger.info("endOperation----" + anaesevent);
     ...
     * @return
     */
    @RequestMapping("/endOperationSYZXYY")
    @ResponseBody
    @ApiOperation(value = "正常结束手术", httpMethod = "POST", notes = "正常结束手术")
    public String endOperationSYZXYY(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start endOperation------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        logger.info("endOperation----" + anaesevent);
     ...
     * @return
     */
    @RequestMapping("/backPreoperative")
    @ResponseBody
    @ApiOperation(value = "返回术前", httpMethod = "POST", notes = "返回术前")
    public String backPreoperative(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start backPreoperative------------------------");
        ResponseValue res = new ResponseValue();
        String regOptId = formBean.getRegOptId();
        myOperationDataService.backPreoperative(regOptId, res);
        CmdMsg msg = new CmdMsg();
        msg.setMsgType(MyConstants.OPERATION_STATUS_END);
        msg.setRegOptId(regOptId);
        res = MessageProcess.process(msg);
        logger.info("------------------end backPreoperative------------------------");
        return res.getJsonStr();
    }
	/**
     ...
     * @return
     */
    @RequestMapping("/forceEndOperQNZZY")
    @ResponseBody
    @ApiOperation(value = "强制结束手术", httpMethod = "POST", notes = "强制结束手术")
    public String forceEndOperQNZZY(@ApiParam(name = "params", value = "参数") @RequestBody CmdMsg msg) {
        logger.info("----------------start forceEndOper------------------------");
        ResponseValue res = new ResponseValue();
        BaseInfoQuery baseQuery = new BaseInfoQuery();
        //baseQuery.setOperRoomId(Global.getConfig("roomId").toString());
        ...
        if (operaPatrolList.size() > 0) {
            ...
            BasCollectConfig collectConfig = basCollectConfigService.selectByRoomId(baseQuery.getOperRoomId());
            collectConfig.setPatientId("");
            basCollectConfigService.update(collectConfig);
	...
	public String getMonitorConfig(@ApiParam(name = "params", value = "参数") @RequestBody Map map) {
		...
		List<BasMonitorConfig> monitorConfigList = basMonitorConfigService.queryAllMonitorConfig(regOptId);
