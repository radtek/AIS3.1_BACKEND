...
public class OperationMgr {
	...
	public static boolean init(String operId, String bedId) {
		...
		if(!SocketClient.futureMap.containsKey(ip+":"+port)){
			
			logger.info("doConnect---"+ip+":"+port);
			
			if(null != socket.connectMap.get(ip+":"+port)){
				socket.connectMap.remove(ip+":"+port);
			}
		...
		logger.info("init---------初始化bedId:"+bedId);
		
		//logger.info("init---------初始化list:"+list.size());
...
//		}
	...
	public static void endAnalepsia(String bedId, String operId) {
		...
		if (null != socket) {
			...
			// 通过bedID 获取端口和IP
			BasRegionBed regionBed = anaesPacuSocketService.getRegionBed(bedId);
			String key = regionBed.getIpAddr()+":"+regionBed.getPort();
			
			socket.closeFutureChannel(key);
...
//			}
	...
	public static boolean setOperMonitor(String bedId, Map<String, Observe> observes) {
		logger.info("setOperMonitor------设置并发送监测项指令");
		JSONObject json = new JSONObject();
		json.put("msgType", MyConstants.COMMAND_OPERATION_MONITOR);
		json.put("observes", observes.values());
		//SocketClient socket = socketMap.get(bedId);
		return socket.sendMsg(json.toString(),getChannelFuture(bedId));
	}
	...
	public static ChannelFuture getChannelFuture(String bedId){
		
		AnaesPacuSocketService anaesPacuSocketService = SpringContextHolder.getBean(AnaesPacuSocketService.class);
		// 通过bedID 获取端口和IP
		BasRegionBed regionBed = anaesPacuSocketService.getRegionBed(bedId);
		
		return SocketClient.futureMap.get(regionBed.getIpAddr()+":"+regionBed.getPort());
	}
