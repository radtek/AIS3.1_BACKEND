...
public class BasAnaesMedicineStorageController extends BaseController
{
	...
	/**
     * @discription 根据库存统计中的计价用量，查询术中收费药品信息
     * @author pengqing
     * @created 2017年6月13日 下午3:01:40
     * @param systemSearchFormBean
     * @return
     */
    @RequestMapping(value = "/queryChargeAmountDetailByMedId")
    @ResponseBody
    @ApiOperation(value = "根据库存统计中的计价用量，查询术中药品项目信息", httpMethod = "POST", notes = "根据库存统计中的计价用量，查询术中药品信息")
    public String queryChargeAmountDetailByMedId(
        @ApiParam(name = "systemSearchFormBean", value = "系统查询参数") @RequestBody SystemSearchFormBean systemSearchFormBean)
    {
        logger.info("begin queryChargeAmountDetailByMedId");
        ResponseValue resp = new ResponseValue();
        List<BasChargeAmountDetailRecordFormBean> resultList = basAnaesMedicineStorageService.queryChargeAmountDetailByMedId(systemSearchFormBean);
        resp.put("resultList", resultList);
        logger.info("end queryChargeAmountDetailByMedId");
        return resp.getJsonStr();
    }
    
    /**
     * @discription 根据时间统计记账药品总数
     * @author liukui
     * @created 2018年7月13日 下午3:01:40
     * @param map
     * @return
     */
    @RequestMapping(value = "/cntMedicineUseByOperat")
    @ResponseBody
    @ApiOperation(value = "根据时间统计记账药品总数", httpMethod = "POST", notes = "根据时间统计记账药品总数")
    public String cntMedicineUseByOperat(
        @ApiParam(name = "map", value = "根据时间统计记账药品总数")@RequestBody SystemSearchFormBean systemSearchFormBean)
    {
        logger.info("begin cntMedicineUseByOperat");
        ResponseValue resp = new ResponseValue();
        List<BasAnaesMedicineOutRecordFormBean> resultList = basAnaesMedicineStorageService.cntMedicineUseByOperat(systemSearchFormBean);
        int total = basAnaesMedicineStorageService.cntMedicineUseByOperatTotal(systemSearchFormBean);
        resp.put("resultList", resultList);
        resp.put("total", total);
        logger.info("end cntMedicineUseByOperat");
        return resp.getJsonStr();
    }
}
